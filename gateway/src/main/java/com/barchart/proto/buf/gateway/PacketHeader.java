...
/**
 * fast {@link NewsPacket} header peek (100 nanos)
 * 
 * note: relies on {@link NewsPacket#TYPE_FIELD_NUMBER} as field search stop
 */
public final class PacketHeader {
	...
	public static PacketHeader from(final byte[] array) throws Exception {
		...
		while (true) {
			...
			if (number == EOF || number >= GatewayPacket.TYPE_FIELD_NUMBER) {
				return header;
			}

			if (number == GatewayPacket.CHANNEL_FIELD_NUMBER) {
				header.channel = input.readSInt32();
				header.hasChannel = true;
			}

			if (number == GatewayPacket.SEQUENCE_FIELD_NUMBER) {
				header.sequence = input.readSInt64();
				header.hasSequence = true;
			}

			if (number == GatewayPacket.TIMESTAMP_FIELD_NUMBER) {
