...
public final class PacketHeader {
	...
	public static PacketHeader from(final byte[] array) throws Exception {
		...
		while (true) {
			...
			if (number == EOF || number >= NewsPacket.TYPE_FIELD_NUMBER) {
				return header;
			}

			if (number == NewsPacket.CHANNEL_FIELD_NUMBER) {
				header.channel = input.readSInt32();
				header.hasChannel = true;
			}

			if (number == NewsPacket.SEQUENCE_FIELD_NUMBER) {
				header.sequence = input.readSInt64();
				header.hasSequence = true;
			}

			if (number == NewsPacket.TIMESTAMP_FIELD_NUMBER) {
