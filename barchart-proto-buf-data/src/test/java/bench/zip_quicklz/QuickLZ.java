...
public final class QuickLZ {
	...
	static int headerLen(final byte[] source) {
	...
	private static void write_header(final byte[] dst, final int level,
			final boolean compressible, final int size_compressed,
			final int size_decompressed) {
	...
	public static byte[] compress(final byte[] source, final int level) {
		int src = 0;
		int dst = DEFAULT_HEADERLEN + CWORD_LEN;
		long cword_val = 0x80000000L;
		int cword_ptr = DEFAULT_HEADERLEN;
		final byte[] destination = new byte[source.length + 400];
		int[][] hashtable;
		final int[] cachetable = new int[HASH_VALUES];
		final byte[] hash_counter = new byte[HASH_VALUES];
		byte[] d2;
		int fetch = 0;
		final int last_matchstart = (source.length - UNCONDITIONAL_MATCHLEN
				- UNCOMPRESSED_END - 1);
		int lits = 0;
		...
		while (src <= last_matchstart) {
			if ((cword_val & 1) == 1) {
				if (src > 3 * (source.length >> 2) && dst > src - (src >> 5)) {
					...
					write_header(d2, level, false, source.length, source.length
							+ DEFAULT_HEADERLEN);
					System.arraycopy(source, 0, d2, DEFAULT_HEADERLEN,
							source.length);
					return d2;
				}

				fast_write(destination, cword_ptr,
			...
			if (level == 1) {
				...
				final int o = hashtable[hash][0];
				final int cache = cachetable[hash] ^ fetch;
				...
				if (cache == 0
						&& hash_counter[hash] != 0
						&& (src - o > MINOFFSET || (src == o + 1 && lits >= 3
								&& src > 3 && source[src] == source[src - 3]
					...
					if (source[o + 3] != source[src + 3]) {
						final int f = 3 - 2 | (hash << 4);
						destination[dst + 0] = (byte) (f >>> 0 * 8);
						destination[dst + 1] = (byte) (f >>> 1 * 8);
						src += 3;
						dst += 2;
					} else {
						final int old_src = src;
						final int remaining = ((source.length
								- UNCOMPRESSED_END - src + 1 - 1) > 255 ? 255
						...
						final int matchlen = src - old_src;

						hash <<= 4;
						if (matchlen < 18) {
							final int f = hash | (matchlen - 2);
							// Neither Java nor C# wants to inline fast_write
							destination[dst + 0] = (byte) (f >>> 0 * 8);
							destination[dst + 1] = (byte) (f >>> 1 * 8);
							dst += 2;
						} else {
							final int f = hash | (matchlen << 16);
				...
				byte c;
				final int remaining = ((source.length - UNCOMPRESSED_END - src
				...
				for (k = 0; k < QLZ_POINTERS_3 && (c > k || c < 0); k++) {
					o = hashtable[hash][k];
					if ((byte) fetch == source[o]
							&& (byte) (fetch >>> 8) == source[o + 1]
							&& (byte) (fetch >>> 16) == source[o + 2]
							&& o < src - MINOFFSET) {
						m = 3;
				...
				if (matchlen >= 3 && src - o < 131071) {
					...
					if (matchlen == 3 && offset <= 63) {
						...
						fast_write(destination, dst, ((matchlen - 3) << 2)
								| (offset << 6) | 2, 2);
						dst += 2;
					} else if (matchlen <= 33) {
						fast_write(destination, dst, ((matchlen - 2) << 2)
								| (offset << 7) | 3, 3);
						dst += 3;
					} else {
						fast_write(destination, dst, ((matchlen - 3) << 7)
								| (offset << 15) | 3, 4);
						dst += 4;
					}
				} else {
		...
		fast_write(destination, cword_ptr, ((cword_val >>> 1) | 0x80000000L),
				CWORD_LEN);
	...
	static public byte[] decompress(final byte[] source) {
		final int size = (int) sizeDecompressed(source);
		int src = headerLen(source);
		int dst = 0;
		long cword_val = 1;
		final byte[] destination = new byte[size];
		final int[] hashtable = new int[4096];
		final byte[] hash_counter = new byte[4096];
		final int last_matchstart = size - UNCONDITIONAL_MATCHLEN
				- UNCOMPRESSED_END - 1;
		int last_hashed = -1;
		int hash;
		int fetch = 0;
		...
		for (;;) {
			...
			if ((cword_val & 1) == 1) {
				...
				if (level == 1) {
					fetch = (int) fast_read(destination, last_hashed + 1, 3); // destination[last_hashed
																				// +
																				// 1]
																				// |
																				// (destination[last_hashed
																				// +
																				// 2]
																				// <<
																				// 8)
				...
				if (dst <= last_matchstart) {
					...
					if (level == 1) {
						while (last_hashed < dst - 3) {
							last_hashed++;
							final int fetch2 = (int) fast_read(destination,
									last_hashed, 3);
							hash = ((fetch2 >>> 12) ^ fetch2)
									& (HASH_VALUES - 1);
							hashtable[hash] = last_hashed;
							hash_counter[hash] = 1;
						}
