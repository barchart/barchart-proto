...
static inline u64* cv_get_u64_values (fast_codec_t* codec, u32 tag)
{
   return cv_get_tag_state (codec, tag)->u64_values;
}
...
static inline u64 cv_get_u64 (fast_codec_t* codec, u32 tag)
{
   return cv_get_u64_values (codec, tag) [get_tag_slot (tag)];
}
...
static inline int cv_eq_u64 (fast_codec_t* codec, u32 tag, u64 data)
{
   return cv_is_valid (codec, tag) && cv_get_u64 (codec, tag) == data;
}
...
static inline void cv_set_u64 (fast_codec_t* codec, u32 tag, u64 data)
{
   cv_get_u64_values (codec, tag) [get_tag_slot (tag)] = data;
   cv_set_valid (codec, tag);
}
...
static int u64_to_size (u64 data)
{
   if (data < 0x00000080) return 1; // 128
   if (data < 0x00004000) return 2; // 16384
   if (data < 0x00200000) return 3; // 2097152
...
//! @brief Serielize a 64-bit unsigned integer field

static int emit_u64 (fast_codec_t* codec, fast_tag_t tag, u64 data)
{
   int size = u64_to_size (data);
...
static int parse_64 (fast_codec_t* codec, fast_tag_t tag,
                     u64* data, int sign_mask)
{
   u8  buffer [5];
   int bytes;
   int p1; 
...
static int parse_u64 (fast_codec_t* codec, fast_tag_t tag, u64* data)
{
   return parse_64 (codec, tag, data, SIGN_MASK_U32);
}
...
int fast_decode_u64 (fast_codec_t* codec, fast_tag_t tag, u64* value)
{
   fast_op_t op = (fast_op_t)get_tag_op ((fast_op_t)tag);
   ...
   if (check_type  (FUNCTION, codec, tag, FAST_TYPE_U64) < 0)
      return -1;

   switch (op)
   {
    case FAST_OP_NONE: // Value must be present
      bytes = parse_u64 (codec, tag, & next);
   ...
   cv_set_u64 (codec, tag, next);

   *value = cv_get_u64 (codec, tag);
   return 0;
}
...
int fast_encode_u64 (fast_codec_t* codec, fast_tag_t tag, u64 value)
{
   fast_op_t op = (fast_op_t)get_tag_op (tag);
   u64 next = value;
   ...
   cv_set_u64 (codec, tag, next); // update previous value state
   return emit_u64 (codec, tag, value); // emit the value
}
