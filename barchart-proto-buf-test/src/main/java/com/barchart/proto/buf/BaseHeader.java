...
/**
 * fast {@link Base} header peek (100 nanos)
 * 
 * note: relies on {@link Base#TYPE_FIELD_NUMBER} as field search stop
 */
public final class BaseHeader {
	...
	public static BaseHeader from(final byte[] array) throws Exception {

		final BaseHeader header = new BaseHeader();
		...
		while (true) {
			...
			if (number == EOF || number >= Base.TYPE_FIELD_NUMBER) {
				return header;
			}

			if (number == Base.CHANNEL_FIELD_NUMBER) {
				header.channel = input.readSInt32();
				header.hasChannel = true;
			}

			if (number == Base.SEQUENCE_FIELD_NUMBER) {
				header.sequence = input.readSInt64();
				header.hasSequence = true;
			}

			if (number == Base.TIMESTAMP_FIELD_NUMBER) {
